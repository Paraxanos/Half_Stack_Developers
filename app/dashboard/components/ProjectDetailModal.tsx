...
interface Project {
  ...
  owner: ProjectOwner;
  ownerId: string; // ← Must be passed from Dashboard
  tags: string[];
  matchReason: string;
  teamSize?: number;
  maxTeamSize?: number;
...
export default function ProjectDetailModal({
  ...
  const handleRequestMeetup = async () => {
    ...
    try {
      ...
      const proposerName = userDoc.exists() ? userDoc.data().name : 'User';

      await addDoc(collection(db, 'meetups'), {
        projectId: project.id,
        proposerUid: currentUser.uid,
        proposername: proposerName,
        recipientUid: project.ownerId, // ✅ Real UID
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 bg-black/80 transition-opacity" onClick={onClose} />
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-10">
        ...
        >
          <button onClick={onClose} className="absolute right-5 top-5 z-10 group">
            <div className="relative flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.03] border border-white/10 transition-all duration-200 group-hover:bg-[#B19EEF]/10 group-hover:border-[#B19EEF]/30">
              <svg className="h-4 w-4 text-gray-500 transition-all duration-200 group-hover:text-[#B19EEF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </div>
          </button>

          {/* Rest of your modal UI — unchanged */}
            ...
            <div className="flex items-start gap-5">
              <div className="relative flex-shrink-0">
                <svg className="w-20 h-20 -rotate-90" viewBox="0 0 36 36">
                  ...
                  <path className="text-[#B19EEF]" stroke="currentColor" strokeWidth="3" strokeLinecap="round" fill="none" strokeDasharray={`${project.compatibilityScore}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                </svg>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-[#B19EEF] to-[#8B7BD4] text-xs font-semibold text-[#0a0a0f]">
                    {project.owner.avatar ? (
                      <img src={project.owner.avatar} alt={project.owner.name} className="h-full w-full rounded-full object-cover" />
                    ) : (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <span key={tag} className="rounded-lg bg-[#B19EEF]/10 border border-[#B19EEF]/15 px-2.5 py-1 text-xs font-medium text-[#B19EEF]/90">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {project.missingRoles.map((role) => (
                  <span key={role} className="rounded-lg border border-dashed border-[#B19EEF]/40 bg-[#B19EEF]/5 px-3 py-1.5 text-xs font-medium text-[#B19EEF]">
                    + {role}
                  </span>
                ))}
              </div>
          ...
          <div className="sticky bottom-0 border-t border-white/5 bg-[#0c0c12]/95 backdrop-blur-sm p-4">
            <div className="flex gap-3">
              ...
              >
                <FiCalendar size={16} />
                {loading ? 'Sending...' : 'Request Meetup'}
              </button>
            </div>
